...
void cadastrar(int funcionarios[100][6]){
	int matricula, filhos=0, idade,filhos_menores=0, filhos_maiores=0, anos_empresa=0;  //<--A MATRIZ SE ORGANIZA DA SEGUINTE MANEIRA:  i = matricula || idade funcionario ||tempo de empresa ||  número filhos || filhos menores de idade, || maiores de idade
	int i, j;                                                     //   j = numero de funcionários                           [0]           [1]                [2]               [3]                       [4]                   [5] 
		
	printf("Digite a mátricula do funcionário:");
	scanf("%d", &matricula);
...
int listar(int funcionarios[100][6], int *total){
	int j;
...
int main(){
 ...
 do{
   ...
   switch(opcao){
   	...
   	printf("%4d", funcionarios);  //  <--- teste de lógica do armazenamento do módulo cadastrar() "Dione"
   	total++;
   	break;
   	case 2:
